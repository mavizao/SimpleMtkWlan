...
#include "MtkWlanUserClient.hpp"
#include <sys/_netstat.h>

#define super IOUserClient
OSDefineMetaClassAndStructors( MtkWlanUserClient, IOUserClient );

static constexpr int kMtkMinDbm = -100;

const IOControlMethodAction MtkWlanUserClient::sMethods[IOCTL_ID_MAX] {
    sDRIVER_INFO,
    sSTA_INFO,
    sPOWER,
    sSTATE,
...
bool MtkWlanUserClient::initWithTask(task_t owningTask, void *securityID, UInt32 type, OSDictionary *properties)
{
    fTask = owningTask;
    return super::initWithTask(owningTask, securityID, type, properties);
}

bool MtkWlanUserClient::start(IOService *provider)
{
//    IOLog("start\n");
    if( !super::start( provider ))
        return false;
    fDriver = OSDynamicCast(SimpleMtkWlan, provider);
...
IOReturn MtkWlanUserClient::clientClose()
{
//    IOLog("clientClose\n");
    if( !isInactive())
        terminate();
...
IOReturn MtkWlanUserClient::clientDied()
{
//    IOLog("clientDied\n");
    return super::clientDied();
}

void MtkWlanUserClient::stop(IOService *provider)
{
//    IOLog("stop\n");
    super::stop( provider );
}

IOReturn MtkWlanUserClient::externalMethod(uint32_t selector, IOExternalMethodArguments * arguments, IOExternalMethodDispatch * dispatch, OSObject * target, void * reference)
{
...
IOReturn MtkWlanUserClient::
sDRIVER_INFO(OSObject* target, void* data, bool isSet)
{
    MtkWlanUserClient *that = OSDynamicCast(MtkWlanUserClient, target);
    ...
    snprintf(drv_info->driver_version, sizeof(drv_info->driver_version), "%s%s", SMTW_VERSION, GIT_COMMIT);
    return kIOReturnSuccess;
}
...
sSTA_INFO(OSObject* target, void* data, bool isSet)
{
    ...
    st->op_mode = ic->ic_curmode > 0 ? (enum mtk_phy_mode)(ic->ic_curmode - 1) : MTK80211_MODE_11A;
    ...
    st->rssi = -(0 - kMtkMinDbm - ic_bss->ni_rssi);
...
sSTATE(OSObject* target, void* data, bool isSet)
{
    ...
    st->version = IOCTL_VERSION;
    st->state = (mtk_80211_state)that->fDriver->fHalService->get80211Controller()->ic_state;
    return kIOReturnSuccess;
}
...
sSCAN_RESULT(OSObject* target, void* data, bool isSet)
{
    ...
    ni->ni_rsncipher = (enum mtk80211_cipher)that->fNextNodeToSend->ni_rsncipher;
    ...
    ni->rsn_groupcipher = (enum mtk80211_cipher)that->fNextNodeToSend->ni_rsngroupcipher;
    ni->rsn_groupmgmtcipher = (enum mtk80211_cipher)that->fNextNodeToSend->ni_rsngroupmgmtcipher;
    ...
    ni->noise = 0;
    ni->rssi = -(0 - kMtkMinDbm - that->fNextNodeToSend->ni_rssi);
